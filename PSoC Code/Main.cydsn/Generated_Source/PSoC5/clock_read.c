...
/*******************************************************************************
* Function Name: clock_read_Start
********************************************************************************
*
* Summary:
...
void clock_read_Start(void) 
{
    ...
    clock_read_CLKEN |= clock_read_CLKEN_MASK;
	clock_read_CLKSTBY |= clock_read_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: clock_read_Stop
********************************************************************************
*
* Summary:
...
void clock_read_Stop(void) 
{
    ...
    clock_read_CLKEN &= (uint8)(~clock_read_CLKEN_MASK);
	clock_read_CLKSTBY &= (uint8)(~clock_read_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: clock_read_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((clock_read_CLKEN & clock_read_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = clock_read_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = clock_read_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* clock_read__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(clock_read_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(clock_read_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: clock_read_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: clock_read_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = clock_read_GetSourceRegister();
    uint16 oldDivider = clock_read_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = clock_read_CLKEN & clock_read_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((clock_read_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: clock_read_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(clock_read_DIV_PTR);
}


/*******************************************************************************
* Function Name: clock_read_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    clock_read_MOD_SRC |= modeBitMask & (uint8)clock_read_MODE_MASK;
}


/*******************************************************************************
* Function Name: clock_read_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    clock_read_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(clock_read_MODE_MASK));
}


/*******************************************************************************
* Function Name: clock_read_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return clock_read_MOD_SRC & (uint8)(clock_read_MODE_MASK);
}


/*******************************************************************************
* Function Name: clock_read_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = clock_read_GetDividerRegister();
    uint8 oldSrc = clock_read_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        clock_read_MOD_SRC |= CYCLK_SSS;
        clock_read_MOD_SRC =
            (clock_read_MOD_SRC & (uint8)(~clock_read_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: clock_read_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return clock_read_MOD_SRC & clock_read_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: clock_read_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    clock_read_PHASE = clkPhase & clock_read_PHASE_MASK;
}


/*******************************************************************************
* Function Name: clock_read_GetPhase
********************************************************************************
*
* Summary:
...
{
    return clock_read_PHASE & clock_read_PHASE_MASK;
}

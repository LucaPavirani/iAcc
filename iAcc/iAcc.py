...
class SerialWorker(QRunnable):
    ...
    def HR_computation(self, data, calibration_flag):
        ...
        if (calibration_flag):
            ...
            if (var.flagstart_HR ==0):  # allows to obtain a precise timing for the HR definition
                var.start_HR = time.time()
                var.flagstart_HR=1
            ...
            if (var.count_sec_HR==var.SECOND):     # The HR is updated after about 30 s
                ...
                var.zData_array_HR=[]    
                var.zData_windowed_HR = signal.savgol_filter(self.HR_detrend, window_length=11, polyorder=4)
                self.window_length_MA = 11
    ...
    def RR_computation(self, data, calibration_flag):
        ...
        The pre-processing steps applied to the data are the following: 
            ...
            2) Then we apply a Band-Pass filter in the frequency range [0.1 ÷ 1] Hz;
        ...
        if (calibration_flag):
            ...
            if (var.flagstart_RR == 0):     # allows to obtain a precise timing for the RR definition
                var.start_RR = time.time()
                var.flagstart_RR=1
            ...
            if (var.count_sec_RR==var.SECOND):     # The HR is updated after about 30 s
                ...
                var.zData_array_RR=[]    
                self.detrend = self.butter_bandpass_design(var.zData_lowpass_RR, var.LOW_CUT_RR, var.HIGH_CUT_RR,
                                                                                                var.SAMPLE_RATE)

                var.zData_bandpass_RR=signal.detrend(self.detrend)
        
                # Threshold computation
                self.threshold_RR=self.calibration_threshold(var.zData_bandpass_RR)
                self.delta_RR = 46

                # Number of peaks detected in the defined time window
                self.n_peaks_bp_RR, var.i_peaks_RR = self.find_peaks(var.zData_bandpass_RR, self.threshold_RR, self.delta_RR)

                var.stop_RR = time.time()
                var.timediff_RR = var.stop_RR - var.start_RR
                var.stop_RR = 0
                var.start_RR = 0
                var.flagstart_RR=0
                ...
                plt.plot(var.i_peaks_RR,var.zData_bandpass_RR[var.i_peaks_RR], "x")
